...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr)) {
    let el = arr.find(e => e > num)
    if(el > num){
      return el
    }else{
      // Якщо ні повертаємо рядок Числа яке задовільняе умову не знайдено - НЕ ЗРОЗУМІЛО
    }
  } else {
    return undefined
  }
...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr)) {
    let i = arr.findIndex((e) => e['name'] == name)
    return i
  } else {
    return -1
  }
...
function getPositiveArrayValues(arr) {
  ...
  if (Array.isArray(arr)) {
    let i = arr.values(arr)

    // Конвертуємо ітератор в масив. - шось не знаю, як це робити)
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (Array.isArray(arr)) {
    arr = arr.flat()
    arr = arr.map((e) => e * e)
    return arr.filter((e) => e > threshold)
  } else {
    return 'Вхідний параметр має бути масивом'
  }
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr)) {
    arr = arr.slice(-n);
    let sum = arr.reduceRight((a,e) => a + e)
    arr.forEach(e => console.log(e));
    return sum
  } else {
    return 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  ...
  if (Array.isArray(arr)) {
    return arr.flatMap((e) => {let temp = []; temp.length = 3; return temp.fill(e)})
  } else {
...
function findLongestWord(arr) {
  ...
  if (Array.isArray(arr)) {
    let value = arr.reduce((a, e) => (e.length > a.length) ? e : a
    )
    return value
  } else {
    return ''
...
function calculateTotalPrice(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.reduce((a,e) => a + e['price'], 0)
  } else {
    return 0
  }
}
...
function findLastNegativeNumberIndex(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.findLastIndex((e) => e < 0)
  } else {
    return -1
  }
}
